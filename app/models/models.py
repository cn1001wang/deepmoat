...
class DailyBasic(Base):
    ...
    ts_code = Column(String(20), nullable=False, primary_key=True, comment="TS股票代码")
    trade_date = Column(String(20), nullable=False, primary_key=True, comment="交易日期")
    ...
    pe = Column(Float, comment="市盈率（总市值/净利润，亏损的PE为空）")
    pe_ttm = Column(Float, comment="市盈率（TTM，亏损的PE为空）")
...
class Dividend(Base):
    ...
    ts_code = Column(String(100), index=True, comment="TS代码")  # TS代码
    end_date = Column(String(100), index=True, comment="分红年度")  # 分红年度
    ann_date = Column(String(100), index=True, comment="预案公告日")  # 预案公告日
