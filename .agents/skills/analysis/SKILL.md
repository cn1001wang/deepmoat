...
`deepmoat-local-data -> analysis`（必要时补 `tushare-data` 对关键日期做校验）

- 启用 `deepmoat-local-data` 时，必须先查看数据库定义：`app/models/models.py`。
- 启用 `deepmoat-local-data` 时，核心表至少合并：`income`、`balancesheet`、`cashflow`、`daily_basic`。
- 必须给出“价格日期”和“财报日期”。
...
- 文件名：`outputs/reports/{symbol}_{name}/analysis_{symbol}_{name}_{YYMMDDHHmm}.md`
- 代码仅保留 6 位数字，不带 `.SH/.SZ`
- 示例：`analysis_000513_丽珠集团_2604152130.md`
